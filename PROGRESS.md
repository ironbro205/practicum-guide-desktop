...
## 2026-07-22 — v1.0.4 (배너 자동 숨김 제거, PLAN §16) — 릴리스
- 사용자 확정: 배너는 ×·본문 클릭·앱 창 열기로만 닫힘(시간 경과 X). INSTALL.md §8 갱신.

## 2026-07-22 — ★사용자 실기 QA 통과 (세션 마감)
- [x] 자동 업데이트 실전 검증(v1.0.2→1.0.4 자동 갱신) ✅
- [x] 자체 배너(윈도우 설정 무관)·상시 유지·×/클릭 동작 ✅
- [x] 새 공지 1분 알림 + 트레이 깜빡임 ✅
- [x] 숨김/최소화 후 창 열면 새 공지 보임(웹 DesktopRefresher) ✅
- 검사 상태: tsc 0오류·vitest 1,172 통과·CI 빌드 3연속 성공·Codex 리뷰 총 7회 반영.

## 남은 일
- [ ] 교사 배포 안내 — INSTALL.md 내용대로 다운로드 링크 공유(사용자 몫, 개발 작업 없음)
- (아이디어 보류) 새 글 감지 즉시 화면 갱신 신호(pg:content-changed) — 실사용 아쉬우면 그때

_다음 세션 재개: "실습가이드 데스크톱 이어서" — 개발 완료 상태, 남은 건 교사 배포 안내뿐. 수정 요청 시 버전 올리고 태그 푸시하면 자동 업데이트로 배포됨(릴리스 절차는 CLAUDE.md)._
